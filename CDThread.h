...
//// Modify on 12/03/2019 to be adapted at XC8 compiler                 ////
//// File: cdthread.h                                                   ////
//// Description:                                                       ////
...
int cdthread_new(cdThreadID_t* pThId, cdtreadFunctionType ptrFunction, const char * pLogName); //!< creates a new thread for Thread_Engine; after creation the thread is disabled, doesn't have messages and the firsttime is set to true
//sint_t cdthread_newUserManaged(cdThreadID_t* pThId, cdtreadFunctionType ptrFunction); 
int cdthread_newUserManaged(cdThreadID_t* pThId, cdtreadFunctionType ptrFunction, const char * pLogName); //!<creates a new USER managed thread (not managed by Thread Engine); after creation the thread is disabled, doesn't have messages and the first time is set to true
...
int cdthread_isDestroyed(cdThreadID_t pThId);         //!<checks if passed id is destroyed (invalid pThId or pThId signed as destroyed) and if it 
...
////DEPRECATED 2019-03-10: int  cdthread_reassign(cdThreadID_t* pThId, cdtreadFunctionType ptrFunction);    //!<destroys allocated thread and assign a new one

int cdthread_signAsFirtsTime(cdThreadID_t pThId);       //!<set firsttime flag as true for indicated thread
int cdthread_Enable(cdThreadID_t pThId); //!< enable indicated Thread with base/standard priority 
int cdthread_Enable_with_priority(cdThreadID_t pThId, int pPriority); //!< enable indicated Thread with indicated priority 
int cdthread_ISR_FAST_enable(cdThreadID_t pThId);    //!< used only in managed thread and inside ISR to enable a thread without checks and test but quickly
...
#define NEW_CDTHFUNCTION( XyX ) 	int XyX (int firstTime, cdThreadID_t ThisThread, int prevExitCode)
...
#define NEW_CDTHSUBFUNCTION( XyX ) 	int XyX (int firstTime, cdThreadID_t ThisThread, int prevExitCode, void* callerDataPtr)
...
extern  void ForceTim1Value( uint16_t tvalue ); //!< the function used to update timer0 value without tick error in Update_Absolute_Timer
...
#define CDTIMER_1s_TICKS    (_XTAL_FREQ /256)
#define CDTIMER_ms_TICKS(XyX)   (((( XyX * CDTIMER_1s_TICKS)/125)+3)/8)
#define CDTIMER_1ms_TICKS   CDTIMER_ms_TICKS( 1 )



//#define cdtimer_TicksForSeconds( ValXZYX )   (( (cdTimerID)ValXZYX * CDTIMER_1s_TICKS ) + ( ValXZYX >>1 ))
#define cdtimer_TicksForSeconds( ValXZYX )   (( (cdTimerID)ValXZYX * CDTIMER_1s_TICKS ) )
...
#define cdtimer_setupms( msValXZX )          (Absolute_timer + CDTIMER_ms_TICKS( msValXZX ) )
