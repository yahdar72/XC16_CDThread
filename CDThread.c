...
void cdthread_initAll(void){
   ...
   for(idx=0; idx < CDTHREAD_MAX_NUM_THREADS; idx++){
      ...
      cdthreadsSystemArray[idx].LogName[0] = 0;
      cdthreadsSystemArray[idx].LogName[1] = 0;
      cdthreadsSystemArray[idx].LogName[2] = 0;
      cdthreadsSystemArray[idx].LogName[3] = 0;
   }
}
...
/*! \fn int cdthread_new(cdThreadID_t* pThId, cdtreadFunctionType ptrFunction, char * pLogName)
   \author Dario Cortese
   ...
   \version 1.01
*/
int cdthread_new(cdThreadID_t* pThId, cdtreadFunctionType ptrFunction, const char * pLogName){
...
/*! \fn int cdthread_newUserManaged(cdThreadID_t* pThId, cdtreadFunctionType ptrFunction, char * pLogName)
   \author Dario Cortese
   \date 08-08-2012, mod 10-Mar-2019
   ...
   \version 1.01
*/
int cdthread_newUserManaged(cdThreadID_t* pThId, cdtreadFunctionType ptrFunction, const char * pLogName){
...
/*! \fn int cdthread_ISR_FAST_enable(cdThreadID_t pThId)
   \author Dario Cortese
...
/*! \fn int cdthread_Enable(cdThreadID_t pThId)
   \author Dario Cortese
   \date 15-09-2019 updated 15-09-2019
   ...
   \version 1.00
*/
int cdthread_Enable(cdThreadID_t pThId){
   cdThreadID_t realidx;
   //convert pThId in real index for cdthreadsSystemArray
   realidx = cdthread_getArrayIdxFromID(pThId);
   //if an error happened then return false
   ...
   cdthreadsSystemArray[(unsigned)realidx].Priority= CDTHREADPRIORITY_ENABLED; 
   //cdthreadsSystemArray[(unsigned)realidx].Wakeup = CDTHREADWAKEUP_DISABLED;
   return TRUE;
} 

/*! \fn int cdthread_Enable_with_priority(cdThreadID_t pThId, int pPriority)
   \author Dario Cortese
   ...
   \version 1.00
*/
int cdthread_Enable_with_priority(cdThreadID_t pThId, int pPriority){
   cdThreadID_t realidx;
   //convert pThId in real index for cdthreadsSystemArray
   realidx = cdthread_getArrayIdxFromID(pThId);
   //if an error happened then return false
...
/*! \fn int cdthread_isWaitingMessage(cdThreadID_t pThId)
   \author Dario Cortese
   ...
   \version 1.00
*/
int cdthread_isWaitingMessage(cdThreadID_t pThId){  
   cdThreadID_t realidx;
   //convert pThId in real index for cdthreadsSystemArray
   realidx = cdthread_getArrayIdxFromID(pThId);
   //if an error happened then return false
   ...
   if( cdthreadsSystemArray[(unsigned)realidx].Wakeup != CDTHREADWAKEUP_BYMESSAGE){
      return FALSE;
   }
   return TRUE;
}
